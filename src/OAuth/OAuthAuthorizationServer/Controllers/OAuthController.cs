namespace OAuthAuthorizationServer.Controllers
{
    ...
    public class OAuthController : Controller
    {
        ...
        {
            var pendingRequest = _authorizationServer.ReadAuthorizationRequest();
            if (pendingRequest == null)
            {
                throw new HttpException((int)HttpStatusCode.BadRequest, "Missing authorization request.");
            }

            var requestingClient = MvcApplication.DataContext.Clients.First(c => 
                c.ClientIdentifier == pendingRequest.ClientIdentifier);

            // Consider auto-approving if safe to do so.
            if (((OAuth2AuthorizationServer)_authorizationServer.AuthorizationServerServices).CanBeAutoApproved(pendingRequest))
            {
                var approval = _authorizationServer.PrepareApproveAuthorizationRequest(pendingRequest, HttpContext.User.Identity.Name);
                
                return _authorizationServer.Channel.PrepareResponse(approval).AsActionResult();
            }
        ...
        public ActionResult AuthorizeResponse(bool isApproved)
        {
            ...
            if (isApproved)
            {
                ...
                client.ClientAuthorizations.Add(new ClientAuthorization
                    {
